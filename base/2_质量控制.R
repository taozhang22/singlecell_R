...
p <- p1 + p2; p
ggsave(plot = p, filename = paste0(dir, "/qc_featureScatter.pdf"), width = 60, height = 20, units = "cm")

p <- VlnPlot(object = seurat, features = c("nCount_RNA", "nFeature_RNA", "percent.mt"), layer = "counts", ncol = 1); p
ggsave(plot = p, filename = paste0(dir, "/qc_vlnPlot.pdf"), width = 0.6 * length(unique(seurat$Sample)), height = 20)
